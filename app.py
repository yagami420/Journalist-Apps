...
# SEARCH FOR 1.jpg / 1.jpeg / 1.png ACROSS DISK
go_icon_b64 = ""
possible_files = ["1.jpg", "1.jpeg", "1.png", "1.JPG", "1.JPEG", "1.PNG"]
search_dirs = [os.path.dirname(__file__) if "__file__" in locals() else "", os.getcwd(), "."]
        ...
        if os.path.exists(path) and os.path.isfile(path):
            try:
                with open(path, "rb") as f:
                    go_icon_b64 = base64.b64encode(f.read()).decode()
                break
            except Exception:
                pass
    if go_icon_b64:
...
# HIGH RESOLUTION TORNADO DATA URI FALLBACK
if not go_icon_b64:
    fallback_svg = """<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><path fill='%2306b6d4' d='M10 20 Q50 5 90 20 Q50 35 10 20 Z'/><path fill='%230ea5e9' d='M20 40 Q50 28 80 40 Q50 52 20 40 Z'/><path fill='%230284c7' d='M30 60 Q50 50 70 60 Q50 70 30 60 Z'/><path fill='%230369a1' d='M40 78 Q50 70 60 78 Q50 85 40 78 Z'/><path fill='%230f172a' d='M46 90 Q50 86 54 90 Q50 94 46 90 Z'/></svg>"""
    image_url_css = f"data:image/svg+xml;utf8,{fallback_svg}"
else:
    image_url_css = f"data:image/jpeg;base64,{go_icon_b64}"
...
st.markdown(f"""
    <style>
        ...
        /* DEFAULT ACTION BUTTON (FOR LOGIN) */
        div.stButton > button {{
        ...
        /* HIGH SPECIFICITY STYLING FOR THE TORNADO GO BUTTON */
        .go-btn-wrapper button {{
            background-image: url('{image_url_css}') !important;
        ...
        .go-btn-wrapper button:hover {{
            transform: scale(1.18) !important;
            background-color: transparent !important;
            border: none !important;
            box-shadow: none !important;
        }}
        .go-btn-wrapper button p, .go-btn-wrapper button div, .go-btn-wrapper button span {{
            display: none !important;
            visibility: hidden !important;
            opacity: 0 !important;
        }}
...
# --- 7. RENDER MAIN WORKSPACE (DYNAMIC ARCHIVE SIDEBAR) ---
    ...
    with main_col:
        ...
        def render_copiable_box(key_id, label, text, height=160):
            ...
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin-bottom: 25px; background-color: #ffffff; text-align: right; direction: rtl; width: 100%;">
                ...
                <textarea id="{key_id}_text" style="width: 100%; height: {height}px; padding: 18px; border: 1px solid #dfe1e5; border-radius: 12px; font-size: 17px; box-sizing: border-box; background-color: #ffffff; color: #202124; font-family: 'Jameel Noori Nastaleeq', 'Noto Nastaleeq Urdu', Tahoma, sans-serif; line-height: 2.2; text-align: right; direction: rtl; resize: none; box-shadow: 0 1px 6px rgba(32,33,36,0.04);" readonly>{safe_text}</textarea>
                <div style="text-align: right; margin-top: 8px;">
                    <button id="btn_{key_id}" onclick="copyText_{key_id}()" style="padding: 10px 24px; background-color: #1a73e8; border: 1px solid #1a73e8; border-radius: 18px; font-size: 12px; color: #ffffff; cursor: pointer; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">Copy {label}</button>
                </div>
            </div>
            <script>
                function copyText_{key_id}() {{
                    var copyText = document.getElementById("{key_id}_text");
                    copyText.select();
        ...
        if st.session_state.breaking_story or st.session_state.headline or st.session_state.package:
            ...
            render_copiable_box("breaking", "Breaking Story (بریکنگ اسٹوری)", st.session_state.breaking_story, height=140)
            render_copiable_box("headline", "Headline (ہیڈ لائن)", st.session_state.headline, height=200)
            render_copiable_box("package", "Package (پیکج)", st.session_state.package, height=360)
            render_copiable_box("cgs", "CGs / Lower Thirds (سی جی / لوئر تھرڈز)", st.session_state.cgs, height=180)