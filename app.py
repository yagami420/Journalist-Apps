...
st.markdown("""
    <style>
        ...
        /* Style Search Input Box & Login Inputs - Sleek Editorial Search Bar with Premium Gray Border */
        ...
        /* Style Go & Regular Buttons - Premium High-End Slate and Outline Styling */
        div.stButton > button {
            border-radius: 12px !important;
        ...
        div.stButton > button[key="guest_login_btn"] {
            ...
            box-shadow: 0 4px 14px rgba(79, 70, 229, 0.05) !important;
        }
...
# Add persistent "Powered by HAFtech" top left button
...
if not st.session_state.logged_in:
    ...
    with login_center_col2:
        ...
        # Premium Styled Login Form Containers
        st.markdown("<h4 style='text-align: center; color: #475569; font-weight: 700; margin-bottom: 20px;'>PORTAL SIGN IN</h4>", unsafe_allow_html=True)
        
        username_input = st.text_input("Username", placeholder="Enter username...", label_visibility="collapsed")
        password_input = st.text_input("Password", placeholder="Enter password...", type="password", label_visibility="collapsed")
        
        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
        
        # Standard Login authentication trigger
        if st.button("LOG IN", type="primary"):
            # Mock secure authentication check
            if username_input.strip().lower() == "admin" and password_input == "jalbagola":
                st.session_state.logged_in = True
                st.session_state.error = ""
                st.rerun()
            else:
                st.error("Invalid Username or Password. Use 'Guest Login' or register.")
                
        # Premium Spacing Divider
        st.markdown("<p style='text-align: center; color: #94a3b8; font-size: 12px; margin: 15px 0;'>OR</p>", unsafe_allow_html=True)
        
        # Guest Login Bypass Mechanism
        if st.button("GUEST LOGIN", key="guest_login_btn"):
            st.session_state.logged_in = True
            st.session_state.error = ""
            st.rerun()
...
# --- 7. RENDER MAIN WORKSPACE (WHEN LOGGED IN) ---
    ...
    # 2.8 Units for Archive Panel (Left), 7.2 Units for main workspace (Right)
    archive_col, main_col = st.columns([2.8, 7.2], gap="large")

    # --- LEFT COLUMN: ARCHIVE SIDEBAR PANEL ---
    with archive_col:
        # Stylized vertical divider and section header - Border on the right to divide cleanly
        st.markdown("""
            <div style="border-right: 2px solid #f1f5f9; padding-right: 15px; margin-right: 10px; text-align: left;">
    ...
    with main_col:
        ...
        def render_copiable_box(label, text, height=120):
            ...
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin-bottom: 20px; background-color: #fafbfc; text-align: right; direction: rtl;">
                ...
                <textarea id="{label}_text" style="width: 100%; height: {height}px; padding: 14px; border: 1.5px solid #cbd5e1; border-radius: 12px; font-size: 17px; box-sizing: border-box; background-color: #ffffff; color: #0f172a; font-family: 'Jameel Noori Nastaleeq', 'Noto Nastaleeq Urdu', 'Urdu Typesetting', Tahoma, sans-serif; line-height: 2.0; text-align: right; direction: rtl; resize: none; box-shadow: 0 4px 14px rgba(0,0,0,0.015); transition: all 0.2s ease;" readonly>{text}</textarea>
                <div style="text-align: right; margin-top: 8px; direction: rtl;">
                    <button id="btn_{label}" onclick="copyText_{label}()" style="padding: 10px 24px; background-color: #0f172a; border: 1px solid #0f172a; border-radius: 12px; font-size: 12px; color: #ffffff; cursor: pointer; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-weight: 700; transition: all 0.2s ease; box-shadow: 0 4px 12px rgba(15, 23, 42, 0.08); text-transform: uppercase; letter-spacing: 0.5px;">Copy {label}</button>
                </div>
            </div>
            <script>
