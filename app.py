...
# --- 3. GOOGLE GEMINI PROCESSOR (INSPECTING RAW RESPONSE METADATA) ---
def process_with_gemini(text, api_key):
    ...
    system_prompt = (
        ...
        "جب میں آپ کو کوئی مواد دوں تو آپ کو درج ذیل 4 آؤٹ پٹ تیار کرنے ہیں:\n"
        "1. بریکنگ اسٹوری (breaking_story): روان اور جاندار بریکنگ اسٹوری۔\n"
        "2. ہیڈ لائن (headline): تفصیلی اور جامع اردو ہیڈ لائن (کم از کم 4 سے 5 جملے۔ تمام اعداد اردو الفاظ میں لکھیں، کوئی انگریزی نہ ہو)۔\n"
        "3. پیکج (package): مکمل ٹی وی پیکیج (اوسی، PKG, MONTAGE, وائس اور، TEMPS)۔\n"
        "4. سی جی / لوئر تھرڈز (cgs): 3 سے 5 کامل سی جی لائنز۔\n\n"
        "سخت ایڈیٹوریل قواعد: املا 'امریکا'، 'صورت حال'، 'فٹ بال'، 'کے لیے' الگ لکھیں۔ جواب صرف اردو میں اور JSON میں فراہم کریں۔\n"
        "{\n"
        "  \"breaking_story\": \"...\",\n"
        "  \"headline\": \"...\",\n"
        "  \"package\": \"...\",\n"
    ...
    for model_name in targets:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 🔍 EXTRACT EXACT METADATA FROM API RESPONSE OBJECT
                raw_model_version = result.get("modelVersion", model_name)
                parsed_json["_actual_model_version"] = raw_model_version
                return parsed_json
            else:
                last_err = f"{model_name} -> {response.status_code}: {response.text}"
        except Exception as e:
            last_err = str(e)
