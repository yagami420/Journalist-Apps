...
# --- 2. ENHANCED WEB SCRAPER WITH ANTI-BLOCKING & FALLBACK ---
def scrape_article(url):
    try:
        ...
        # Strategy A: Target Main Article or Main Text Containers
        ...
        # Strategy B: Fallback to Body text if paragraphs yield very little text
...
# --- 4. EXPORT ARCHIVE TO BEAUTIFUL HTML ---
...
st.markdown("""
    <style>
        ...
        /* EXPANDED CONTAINER WIDTH FOR FULL SPACIOUS WORKSPACE */
        .block-container {
...
# --- 6. RENDER SECURE GATE (LOGIN PAGE - NO GUEST ACCESS) ---
if not st.session_state.logged_in:
    ...
    with login_center_col2:
        ...
        st.markdown("<div style='background-color: #ffffff; padding: 45px; border-radius: 12px; border: 1px solid #e2e8f0; box-shadow: 0 10px 30px rgba(0,0,0,0.02);'>", unsafe_allow_html=True)
        
        # ENLARGED & CENTERED LOGO
        if os.path.exists(logo_path):
            with open(logo_path, "rb") as img_file:
                img_b64 = base64.b64encode(img_file.read()).decode()
            st.markdown(f'<div style="display: flex; justify-content: center; align-items: center; margin-bottom: 30px; width: 100%;"><img src="data:image/png;base64,{img_b64}" style="max-width: 380px; width: 100%; border-radius: 10px;" /></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="display: flex; justify-content: center; align-items: center; margin-bottom: 30px; width: 100%;"><div style="text-align: center; padding: 30px; border-radius: 12px; background: linear-gradient(135deg, #0f172a, #1e293b); color: #ffffff; max-width: 380px; width: 100%;"><h1 style="margin: 0; font-family: Georgia, serif; font-size: 2.5rem; color: #38bdf8;">🌪️ جل بگولہ</h1><p style="margin: 5px 0 0 0; font-size: 0.85rem; color: #94a3b8; text-transform: uppercase;">EDITORIAL GATEWAY</p></div></div>', unsafe_allow_html=True)
...
# --- 7. RENDER MAIN WORKSPACE (WHEN LOGGED IN) ---
    ...
    # WIDER MAIN COLUMN RATIO (2.2 : 7.8)
    archive_col, main_col = st.columns([2.2, 7.8], gap="large")
    ...
    with main_col:
        ...
        # ENLARGED & CENTERED LOGO IN WORKSPACE
        if os.path.exists(logo_path):
            with open(logo_path, "rb") as img_file:
                img_b64 = base64.b64encode(img_file.read()).decode()
            st.markdown(f'<div style="display: flex; justify-content: center; align-items: center; margin-bottom: 30px; width: 100%;"><img src="data:image/png;base64,{img_b64}" style="max-width: 380px; width: 100%; border-radius: 12px;" /></div>', unsafe_allow_html=True)
        ...
        # EXPANDED COPIABLE BOX FUNCTION FOR WIDER & TALLER TEXT AREAS
        def render_copiable_box(label, text, height=160):
            safe_text = text.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n").replace('"', '\\"')
            html_code = f"""
            <div style="font-family: 'Georgia', serif; margin-bottom: 25px; background-color: #ffffff; text-align: right; direction: rtl; width: 100%;">
                ...
                <textarea id="{label}_text" style="width: 100%; height: {height}px; padding: 18px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 17px; box-sizing: border-box; background-color: #ffffff; color: #0f172a; font-family: 'Jameel Noori Nastaleeq', 'Noto Nastaleeq Urdu', Tahoma, sans-serif; line-height: 2.2; text-align: right; direction: rtl; resize: vertical; box-shadow: 0 1px 10px rgba(0,0,0,0.015);" readonly>{text}</textarea>
        ...
        if st.session_state.breaking_story or st.session_state.headline or st.session_state.package:
            ...
            # LARGER HEIGHTS FOR SPACIOUS DISPLAY
