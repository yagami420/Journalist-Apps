...
# PURE WHITE GOOGLE-LIKE STYLING
st.markdown("""
    <style>
        /* HIDE ALL STREAMLIT DECORATION / TOOLBARS / HEADER ARTIFACTS */
        header, [data-testid="stHeader"], [data-testid="stDecoration"], [data-testid="stStatusWidget"], [data-testid="manage-app-button"] {
            display: none !important;
            height: 0 !important;
            visibility: hidden !important;
        }
        
        /* PURE GOOGLE WHITE BACKGROUND */
        ...
        /* GOOGLE-LIKE LOWER VERTICAL ALIGNMENT */
        .block-container {
...
# --- 6. RENDER SECURE GATE (LOGIN PAGE - LOWER POSITIONED) ---
if not st.session_state.logged_in:
    ...
    with login_center_col2:
        ...
        st.markdown("<div style='background-color: #ffffff; padding: 40px; border-radius: 16px; border: 1px solid #dfe1e5; box-shadow: 0 4px 16px rgba(0,0,0,0.04);'>", unsafe_allow_html=True)
        
        if os.path.exists(logo_path):
            with open(logo_path, "rb") as img_file:
                img_b64 = base64.b64encode(img_file.read()).decode()
            st.markdown(f'<div style="display: flex; justify-content: center; align-items: center; margin-bottom: 25px; width: 100%;"><img src="data:image/png;base64,{img_b64}" style="max-width: 360px; width: 100%;" /></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="display: flex; justify-content: center; align-items: center; margin-bottom: 25px; width: 100%;"><div style="text-align: center; color: #1a73e8;"><h1 style="margin: 0; font-family: Google Sans, Roboto, sans-serif; font-size: 2.8rem;">🌪️ جل بگولہ</h1><p style="margin: 5px 0 0 0; font-size: 0.85rem; color: #5f6368; text-transform: uppercase; letter-spacing: 1px;">EDITORIAL GATEWAY</p></div></div>', unsafe_allow_html=True)
        
        st.markdown("<p style='text-align: center; color: #5f6368; font-size: 13px; font-weight: 600; margin-bottom: 20px; letter-spacing: 0.5px;'>EDITORIAL GATEWAY LOGIN</p>", unsafe_allow_html=True)
...
# --- 7. RENDER MAIN WORKSPACE (GOOGLE CENTERED & SPACIOUS) ---
else:
    ...
    with main_col:
        ...
        if os.path.exists(logo_path):
            with open(logo_path, "rb") as img_file:
                img_b64 = base64.b64encode(img_file.read()).decode()
            st.markdown(f'<div style="display: flex; justify-content: center; align-items: center; margin-bottom: 30px; width: 100%;"><img src="data:image/png;base64,{img_b64}" style="max-width: 360px; width: 100%;" /></div>', unsafe_allow_html=True)
        else:
        ...
        if st.session_state.breaking_story or st.session_state.headline or st.session_state.package:
            ...
            if st.session_state.active_model_version:
                st.markdown(
                    f"""
                    <div style="background-color: #f8f9fa; border: 1px solid #e8eaed; border-radius: 12px; padding: 10px 18px; margin-bottom: 25px; display: flex; align-items: center; justify-content: space-between;">
