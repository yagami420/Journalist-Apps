...
st.markdown("""
    <style>
        ...
        /* Style Search Input Box - Sleek Editorial Search Bar */
        .stTextArea textarea {
...
# Centered Logo Image Section - Perfect Symmetry
...
if os.path.exists(logo_path):
    ...
    st.markdown(
        f"""
        <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 35px; width: 100%;">
            <img src="data:image/png;base64,{img_b64}" style="max-width: 400px; width: 100%; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.04);" />
        </div>
        """, 
        unsafe_allow_html=True
    )
    ...
    st.markdown("""
        <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 35px; width: 100%;">
            <div style="text-align: center; padding: 30px; border-radius: 20px; background: linear-gradient(135deg, #0f172a, #1e293b); color: #ffffff; box-shadow: 0 15px 35px rgba(0,0,0,0.15); max-width: 400px; width: 100%;">
...
# Render premium output text areas with clean outlines, shadows, and unified button aesthetics
def render_copiable_box(label, text, height=120):
    safe_text = text.replace("\\", "\\\\").replace("'", "\\'").replace("\n", "\\n").replace('"', '\\"')
    html_code = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin-bottom: 30px; background-color: #fafbfc; text-align: right; direction: rtl;">
        <label style="font-weight: 700; color: #475569; display: block; margin-bottom: 8px; font-size: 13px; text-transform: uppercase; letter-spacing: 1.5px; text-align: right;">{label}</label>
        <textarea id="{label}_text" style="width: 100%; height: {height}px; padding: 18px; border: 1px solid #e2e8f0; border-radius: 16px; font-size: 21px; box-sizing: border-box; background-color: #ffffff; color: #0f172a; font-family: 'Jameel Noori Nastaleeq', 'Noto Nastaleeq Urdu', 'Urdu Typesetting', Tahoma, sans-serif; line-height: 2.2; text-align: right; direction: rtl; resize: none; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.03); transition: all 0.2s ease;" readonly>{text}</textarea>
        <div style="text-align: right; margin-top: 10px; direction: rtl;">
            <button id="btn_{label}" onclick="copyText_{label}()" style="padding: 10px 24px; background-color: #0f172a; border: 1px solid #0f172a; border-radius: 12px; font-size: 12px; color: #ffffff; cursor: pointer; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-weight: 700; transition: all 0.2s ease; box-shadow: 0 3px 10px rgba(15, 23, 42, 0.12); text-transform: uppercase; letter-spacing: 1px;">Copy {label}</button>
        </div>
    </div>
    <script>
        function copyText_{label}() {{
            ...
            btn.style.boxShadow = "0 3px 10px rgba(16, 185, 129, 0.2)";
            
            setTimeout(function() {{
                btn.innerHTML = "Copy {label}";
                btn.style.backgroundColor = "#0f172a";
                btn.style.borderColor = "#0f172a";
                btn.style.boxShadow = "0 3px 10px rgba(15, 23, 42, 0.12)";
            }}, 1500);
        }}
    </script>
    """
