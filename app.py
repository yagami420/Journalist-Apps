...
# --- 3. GOOGLE GEMINI PROCESSOR (ENHANCED MODEL FALLBACK) ---
def process_with_gemini(text, api_key):
    # Sanitize and clean API Key string
    ...
    # Active Gemini Flash model target list
    targets_to_try = [
    ...
    for api_version, model_name in targets_to_try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                parsed_json["_used_model"] = f"{model_name}"
                return parsed_json
            else:
                errors.append(f"{model_name} (Status {response.status_code})")
        except Exception as e:
            errors.append(f"{model_name} ({str(e)})")
            
    return {"error": f"Gemini API Error: All model targets failed. ({', '.join(errors)}). Please verify 'GEMINI_API_KEY' in Streamlit Secrets or generate a new API Key from AI Studio."}
            ...
            if not GEMINI_API_KEY:
                ...
                with st.spinner("پروسیسنگ جاری ہے..."):
                    ...
                    if "error" in data:
                        ...
                        st.toast(f"⚡ Generated via {used_model}!", icon="🚀")
