...
st.markdown("""
    <style>
        ...
        .viewerBadge, [data-testid="manage-app-button"], .stViewerBadge, iframe[title="manage-app"], div[class*="viewerBadge"], button[class*="viewerBadge"], iframe[src*="manage-app"], div[class*="manage-app"] {
            display: none !important;
            visibility: hidden !important;
            height: 0 !important;
            width: 0 !important;
        ...
        /* Frame the whole app layout to look like an Astra/Divi WordPress Page Template */
        .block-container {
...
if not st.session_state.logged_in:
    ...
    with login_center_col2:
        # Center Logo inside elegant WordPress card structure
...
# --- 7. RENDER MAIN WORKSPACE (WHEN LOGGED IN) ---
    ...
    with archive_col:
        ...
        st.markdown("""
            <div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; box-shadow: 0 4px 18px rgba(0,0,0,0.01); margin-bottom: 25px;">
                <h3 style="color: #1e1b4b; font-family: Georgia, serif; font-size: 17px; font-weight: 700; margin: 0 0 5px 0; border-bottom: 2px solid #1e1b4b; padding-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px;">📋 Recent Posts</h3>
    ...
    with main_col:
        ...
        if os.path.exists(logo_path):
            ...
            st.markdown(
                f"""
                <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 30px; width: 100%;">
                    <img src="data:image/png;base64,{img_b64}" style="max-width: 260px; width: 100%; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.01);" />
                </div>
                """, 
                unsafe_allow_html=True
            )
        ...
        # Render premium output text areas with 16px readable font, clean white cards, and Indigo WP buttons
