...
# --- 3. GOOGLE GEMINI PROCESSOR (MULTI-ENDPOINT & MULTI-MODEL FALLBACK) ---
def process_with_gemini(text, api_key):
    ...
    # Combination of API versions and Flash models to bypass 404 errors completely
    targets_to_try = [
    ...
    for api_version, model_name in targets_to_try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                parsed_json["_used_model"] = f"{model_name} ({api_version})"
                return parsed_json
            else:
                last_err = f"[{model_name} {api_version}] Status {response.status_code}: {response.text}"
        except Exception as e:
            last_err = f"[{model_name} {api_version}] Exception: {str(e)}"
