...
# Automatically fetch the API Key from Streamlit's secure Secrets vault
# This works on Streamlit Cloud and your local machine!
...
def summarize_with_gemini(text, model_choice):
    try:
        ...
        combined_prompt = (
            ...
            "۴. وی او (VO) کو بہت تفصیلی اور طویل بنائیں۔ یہ کم از کم ڈیڑھ سو سے دو سو (150 to 200) الفاظ یا سات سے نو (7 to 9) طویل اور مربوط جملوں پر مشتمل ہونا چاہیے، جس میں واقعے کا پس منظر، تمام کرداروں کے موقف، عدالت یا حکام کی ہدایات اور موجودہ صورتحال کی مکمل تفصیلات روانی کے ساتھ بیان کی گئی ہوں۔\n"
...
# Sidebar status update to reflect secrets loading
if not GEMINI_API_KEY:
    st.sidebar.warning("⚠️ No secure key detected in Streamlit Secrets!")
else:
    st.sidebar.success("🔒 API Key loaded automatically from Streamlit Secrets vault!")
...
if st.button("پیکیج تیار کریں", type="primary"):
    if not GEMINI_API_KEY:
        st.error("Please add your 'GEMINI_API_KEY' in the Streamlit secrets manager.")
