            ...
            if not GROQ_API_KEY:
                ...
                with st.spinner("پروسیسنگ جاری ہے..."):
                    ...
                    if "error" in data:
                        ...
                        verify_status = str(data.get("verification", "")).strip()
                        hl_text = str(data.get("headline", "")).strip()
                        st.session_state.box_headline_verify = f"خبر کی تصدیق:\n{verify_status}\n\nہیڈ لائن:\n{hl_text}"
                        
                        st.session_state.box_package = str(data.get("package", "")).strip()
                        ...
                        cg_str = "\n".join(cg_data) if isinstance(cg_data, list) else str(cg_data).strip()
                        
                        ticker_data = data.get("tickers", "")
                        ticker_text = "\n".join(ticker_data) if isinstance(ticker_data, list) else str(ticker_data).strip()
                        
                        bg_data = data.get("background", "")
                        bg_text = "\n".join(bg_data) if isinstance(bg_data, list) else str(bg_data).strip()
